...
class Kiwoom(QAxWidget):
    ...
    def get_real_data(self,codes):
        for code in codes:
            self.dynamicCall("SetRealReg(QString,QString,QString,QString)", '0101',code,'10;12','1')
    ...
    def _receive_real_data(self, code, realtype, realdata):
        ...
        price = {} , rate = {}                   
        self.price[code] = temp_price
        self.rate[code] = temp_rate        
...
#             earning_rate = Kiwoom.change_format2(earning_rate)
    ...
    def check_balance(self):
        ...
        # balance
        item = QTableWidgetItem(self.d2_deposit)
        item.setTextAlignment(Qt.AlignVCenter | Qt.AlignRight)
        self.tableWidget.setItem(0, 0, item)

        for i in range(1, 6):
            item = QTableWidgetItem(self.opw00018_output['single'][i - 1])
            item.setTextAlignment(Qt.AlignVCenter | Qt.AlignRight)
    ...
    def present_price(self):
        ...
        temp = [] ; price = []
        for code in ['A069500','A364690','A365040']:  # 현재가 객체 구하기
            objStockMst = win32com.client.Dispatch("DsCbo1.StockMst")
            objStockMst.SetInputValue(0, code)   
            objStockMst.BlockRequest()           # 현재가 통신 및 통신 에러 처리 
            rqStatus = objStockMst.GetDibStatus()
            rqRet = objStockMst.GetDibMsg1()
#             print("통신상태", rqStatus, rqRet)
#             if rqStatus != 0:
# #                 exit()
            price.append(objStockMst.GetHeaderValue(11))
            temp.append(objStockMst.GetHeaderValue(12)/objStockMst.GetHeaderValue(11))

        return price[0],price[1],price[2],temp[0],temp[1],temp[2] #전일대비
